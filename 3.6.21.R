### Blockwise bootstrap, Example 3.3.
...
library(boot)
stat <- function(TS){TS}
### one bootstrapped time series, block length=5
TB <- tsboot(TS,stat,sim="fixed",R=1,l=5,n.sim=100,orig.t=FALSE)
### estimator in AR(1)
stat2 <- function(T){arima(T,c(1,0,0),include.mean=F)$coef[1]}
TB2 <- tsboot(TS,stat2,sim="fixed",R=1000,l=5,
